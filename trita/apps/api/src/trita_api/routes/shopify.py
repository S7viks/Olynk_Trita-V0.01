...
def _default_shop() -> str:
    """Partner dev store default when ?shop= omitted (not Yoga Bar's production Shopify)."""
    return (
        os.environ.get("SHOPIFY_DEV_SHOP_DOMAIN")
        or os.environ.get("YOGA_BAR_SHOP_DOMAIN")  # legacy alias
        or "tritabyolynk.myshopify.com"
    ).strip()
...
<p><a href="{start}">Start Shopify connect (step 4)</a></p>
    ...
    upsert_integration_health(
        ...
        detail={
            "connected": True,
            "shop_domain": shop_domain,
            "message": "OAuth complete — run sync for first ingest",
        },
    )

    web_base = os.environ.get("NEXT_PUBLIC_WEB_URL", "http://localhost:3000").strip().rstrip(
        "/"
    )
    if web_base:
